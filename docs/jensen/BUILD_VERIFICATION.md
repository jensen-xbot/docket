...
1. `InlineTaskEditView.swift` (Module 2) — deprecated from flow, kept in project
2. `ChatTaskCard.swift` (Command Bar UX Rework) — elevated task card in conversation
3. `CommandBarVoiceMode.swift` (Module 4)
4. `TodayView.swift` (Module 8)
5. `UpcomingView.swift` (Module 10, optional)
...
**Commit:** TBD  
**Modified files:**
- `supabase/functions/parse-voice-tasks/index.ts` — prompt updates (terminology, time inference, corrections, consolidated questions, closing wording)
- `Docket/Docket/Views/CommandBarExpanded.swift` — title "Task Assistant" → "Ask Docket"
- `Docket/Docket/Views/VoiceRecordingView.swift` — auto-start recording on appear, optimized prefetch sequence

**New documentation:**
- `docs/jensen/VOICE_UX_IMPROVEMENTS_2026-02-14.md` — full changelog

**Expected behavior:**
- Voice sheet opens → recording auto-starts (no mic tap)
- Fewer conversational turns (event/meeting terminology, lunch→noon inference, user corrections)
- Expanded sheet shows "Ask Docket"
